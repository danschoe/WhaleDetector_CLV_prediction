...
from data_processing.utils_transaction_df import (
    group_by_customer,
    map_article_ids,
    train_test_split,
)
...
def group_and_convert_df_pl(
    ...
    agg_df = agg_df.select(
        "customer_id", "days_before_lst", "total_price_lst", "CLV_label"
    )
...
def load_data_group_customers(data_path: Path, group_by_channel_id: bool = False):
    ...
    Returns:
        grouped_df (pl.DataFrame): Processed transaction dataframe
    """
    ...
    df_pl = map_article_ids(df=df_pl)

    grouped_df = group_by_customer(df_pl, group_by_channel_id=group_by_channel_id)
...
def get_customer_train_test_articles_pl(
    ...
    """
    train_end = datetime.strptime(config.get("train_end"), "%Y-%m-%d")
    grouped_df = load_data_group_customers(
        data_path=data_path,
        group_by_channel_id=config.get("group_by_channel_id", False),
    )
